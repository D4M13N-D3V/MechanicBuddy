...
public class DomainVerificationRepository : IDomainVerificationRepository
{
    ...
    public async Task<DomainVerification?> GetByIdAsync(int id)
    {
        ...
        var sql = "SELECT * FROM domain_verifications WHERE id = @Id";
    ...
    {
        using var connection = new NpgsqlConnection(_connectionString);
        var sql = "SELECT * FROM domain_verifications WHERE domain = @Domain ORDER BY created_at DESC LIMIT 1";
    ...
    {
        using var connection = new NpgsqlConnection(_connectionString);
        var sql = "SELECT * FROM domain_verifications WHERE verification_token = @Token";
    ...
    {
        using var connection = new NpgsqlConnection(_connectionString);
        var sql = "SELECT * FROM domain_verifications WHERE tenant_id = @TenantId ORDER BY created_at DESC";
    ...
    {
        using var connection = new NpgsqlConnection(_connectionString);
        var sql = "DELETE FROM domain_verifications WHERE id = @Id";
